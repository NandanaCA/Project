...
def listen1(chat_tree):
    while True:
        ...
        if choice=='1':
            ...
            chat_tree.insert('','end',text=1,values=(username,content,date))
            ...
            chat_tree=ttk.Treeview(chat_frame3,yscrollcommand=chat_scrollbar.set)
            chat_tree.pack(fill=BOTH,expand=YES)

            chat_scrollbar.config(command=chat_tree.yview)

            chat_tree['columns']=("Participants","message","time")
            chat_tree.column("#0",width=0)
            chat_tree.column("Participants",anchor=W,width=150,minwidth=150)
            chat_tree.column("message",anchor=W,width=150,minwidth=150)
            chat_tree.column("time",anchor=W,width=150,minwidth=150)



            chat_tree.heading("#0",text="",anchor=W)
            chat_tree.heading("Participants",text="PARTICIPANTS",anchor=W)
            chat_tree.heading("message",text="MESSAGE",anchor=W)
            chat_tree.heading("time",text="TIME",anchor=W)



            chat_tree.tag_configure('oddrow',background="white")
            chat_tree.tag_configure('evenrow',background="#99bccf")

            chat_tree.insert(parent='',index='end',iid=0,text="",values=(""),tags=('evenrow',))

            chat_frame4=LabelFrame(chat_frame,foreground='#083447')
            chat_frame4.pack(fill="x")

            Label(chat_frame4,text="").pack()
            ...
            for i in history:
                print(i)
                chat_tree.insert('','end',text=1,values=(i[1],i[2],i[3]))
...
def main():
    ...
    Label(chat_frame1,text="",background='#083447').pack()
    Label(chat_frame1,text="",background='#083447').pack()
    b1=Button(chat_frame1,image=history_button,borderwidth=0,command=history)
    b1.place(x=10,y=6)
    b2=Button(chat_frame1,image=search_button,borderwidth=0,command=search)
    b2.place(x=280,y=6)
    ...
    textbox1=Entry(chat_frame4,width=80,font="none 8")
    textbox1.pack()

    threading.Thread(target=listen1,args=(chat_tree,)).start()

    Label(chat_frame4,text="").pack()
    b1=Button(chat_frame4,image=enter_button,borderwidth=0,command=send)
    b1.pack()
